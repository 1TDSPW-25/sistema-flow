                ...
                <form onSubmit={handleSubmit(onSubmit)} className="w-full h-full">
                    <fieldset>
                        ...
                        <div>
                            ...
                            <input type="text" id="idNome"  className="border-2 rounded-[5px] bg-amber-50 p-1 mb-5 w-90" {...register("nome",{required:true, maxLength:50,minLength:3})} 
                            aria-invalid={!!errors.nome} aria-describedby={errors.nome ? "nome-error" : undefined}/>
                            {errors.nome && <span id="nome-error" className="text-red-500 bg-red-300 p-2 rounded border-2 border-red-500 block text-center">Respeite o número de caracteres!</span>}

                        </div>
                        <div> 
                            <label htmlFor="idPreco" className="font-bold block">R$: </label>
                            <input type="number" id="idPreco" step={0.01} className="border-2 rounded-[5px] bg-amber-50 p-1 mb-5 w-90" {...register("preco",{required:true, valueAsNumber:true, min:0.01})} aria-invalid={!!errors.preco} aria-describedby={errors.preco ? "preco-error" : undefined}/>
                            {errors.preco && <span id="preco-error" className="text-red-500 bg-red-300 p-2 rounded border-2 border-red-500 block text-center">Informe um preço válido!</span>}
                        </div>
                        <div>
